...
func part1() {
	...
	for x := 0; x < 50; x++ {
		for y := 0; y < 50; y++ {
			var res int64
			ic := aoc.LoadIntCode(input, aoc.FixedInput(int64(x), int64(y)), aoc.SingleOutput(&res))
			ic.RunToEnd()
			count += int(res)
		}
	}
...
func part2() {
	...
	for y := 100; ; y++ {
		...
		if check(left, y-99) && check(left+99, y) && check(left+99, y-99) {
			fmt.Println(left*10000 + (y - 99))
			return
		}
	}
}

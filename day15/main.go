...
func part1() {
	...
	ic := aoc.LoadIntCode(input, aoc.ChanInput(in), aoc.ChanOutput(out))
	go ic.RunToEnd()
	...
	findPath := func(a, b [2]int) int {
		work := [][3]int{{a[0], a[1], 1}}
		seen := map[[2]int]bool{}
		for len(work) > 0 {
			w := work[0]
			work = work[1:]
			...
			if no, ok := wall[key]; no || !ok {
				continue
			}
			seen[key] = true
			for i, dir := range dirs {
				w2 := [3]int{w[0] + dir[0], w[1] + dir[1], w[2]}
				if key == a {
					w2[2] = i
				}
				work = append(work, w2)
...
outer:
	...
	countPath := func(a, b [2]int) int {
		work := [][3]int{{a[0], a[1], 0}}
		seen := map[[2]int]bool{}
		for len(work) > 0 {
			w := work[0]
			work = work[1:]
	...
	fmt.Println(countPath([2]int{}, endPos))
}

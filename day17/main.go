...
func part1() {
	...
	ic := aoc.LoadIntCode(input, aoc.FixedInput(), write)
	ic.RunToEnd()
	...
	for i := 1; i < hei-1; i++ {
		for j := 1; j < wid-1; j++ {
			...
			for _, p := range [][2]int{{i - 1, j}, {i + 1, j}, {i, j - 1}, {i, j + 1}} {
				if p[0] >= 0 && p[0] < hei && p[1] >= 0 && p[1] < wid && bytes.IndexByte(valid, lines[p[0]][p[1]]) != -1 {
					count++
				}
			}
			if count == 4 {
...
func part2() {
	...
	for i := 0; i < hei; i++ {
		for j := 0; j < wid; j++ {
			idx := bytes.IndexByte(dircs, lines[i][j])
			if idx != -1 {
				pos = [2]int{i, j}
				curdir = idx
			}
	...
	validSpot := func(i, j int) bool {
		return i >= 0 && i < hei && j >= 0 && j < wid && lines[i][j] == '#'
	}

	for {
		dir := dirs[curdir]
		i2, j2 := pos[0]+dir[0], pos[1]+dir[1]
		run := 0
		for validSpot(i2, j2) {
			run++
			pos = [2]int{i2, j2}
		...
		if run > 0 {
			cmds = append(cmds, ',')
			cmds = strconv.AppendInt(cmds, int64(run), 10)
		}
		d1 := dirs[(curdir+1)%4]
		d2 := dirs[(curdir+3)%4]
		if validSpot(pos[0]+d1[0], pos[1]+d1[1]) {
			cmds = append(cmds, ',', 'R')
			curdir = (curdir + 1) % 4
		} else if validSpot(pos[0]+d2[0], pos[1]+d2[1]) {
			cmds = append(cmds, ',', 'L')
			curdir = (curdir + 3) % 4
		} else {
			break
	...
	A := []byte(`L,12,L,12,L,6,L,6`)
	B := []byte(`R,8,R,4,L,12`)
	C := []byte(`L,12,L,6,R,12,R,8`)
	Main := []byte(`A,B,A,C,B,A,C,B,A,C`)

	var in []int64
	for _, b := range Main {
		in = append(in, int64(b))
	}
	in = append(in, 10)
	for _, b := range A {
		in = append(in, int64(b))
	...
	in = append(in, 10, 'n', 10)

	ic = aoc.LoadIntCode(input, aoc.FixedInput(in...), func(v int64) {
		if v > 256 {
			fmt.Println(v)
		}
	})
